...
const skeletonPath = isGenerateSkeleton
  ? path.join(componentDir, `${componentName}Skeleton.tsx`)
  : '';
...
const index =
  ...
  (isGenerateSkeleton
    ? `export { default as ${componentName}Skeleton } from './${componentName}Skeleton';\n`
    : '');
...
console.log(
  `'${componentName}' has been created at ${srcPath + '/' + componentDirPath}`
);
